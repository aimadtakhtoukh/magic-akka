package edition.implementation.scrappers.languages
...
import edition.domain.Models.{Language, SetName, EditionNames}
import edition.domain.scrappers.ScrapperSource
import edition.implementation.scrappers.languages.extractor.{FeaturedInformationExtractor, InfoSetOnPageExtractor, TabNameExtractor, TitleExtractor}
...
class MagicCardSetArchiveScrapperSource(implicit system: ActorSystem, executionContextExecutor: ExecutionContextExecutor)
  ...
  def source : Source[EditionNames, NotUsed] =
    Source.combine(newCardSetProductElements, legacyCardSetArchiveElements)(Concat(_))
      ...
      .map {_ map { case (lang, link) => (lang, link.requestToDocument())}}
    ...
    Flow[Document]
      .map {
        document =>
          ...
          altLinks match {
            case List() =>
              val canonicalLink = document.selectFirst("link[rel='canonical']").attr("href")
              languagesCodes
